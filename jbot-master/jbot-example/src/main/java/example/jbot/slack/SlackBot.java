...
public class SlackBot extends Bot {
    ...
    public void onReceiveDM(WebSocketSession session, Event event) {
    	String toReply = "";
    	if(event.getText().contains("Python")) {
    		String pythonScriptName =  "";
    		pythonScriptName = event.getText().substring(event.getText().indexOf(":")+1);
    		pythonScriptName = pythonScriptName.trim();
    		File file = new File(pythonRootDir+pythonScriptName+".py");
    		if(file.exists()) {
    			Process p;
				try {
					String fileName = file.getAbsolutePath();
					p = Runtime.getRuntime().exec("py "+fileName);
    	...
    	if(event.getText().contains("Feature")) {    		
    		//String javaJarName = event.getText().substring(event.getText().indexOf(":")+1);
    		//javaJarName = javaJarName.trim();
    		String tagName = event.getText().substring(event.getText().indexOf(":")+1);
    		tagName  = tagName.trim();
    		File file = new File(javaFileName);
    		if(file.exists()) {
    			Process p;
				try {
					p = Runtime.getRuntime().exec("java -jar "+file.getAbsolutePath() + " "+tagName);
					BufferedReader stdInput = new BufferedReader(new 
	    	                 InputStreamReader(p.getInputStream()));
    	...
    	Message msg;
    	if(toReply.length() != 0)
    		msg = new Message(toReply);
    	else
    		msg = new Message("Hi, I am " + slackService.getCurrentUser().getName());
